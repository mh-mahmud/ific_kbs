        ...
        <div class="main-content-wrapper w-100 position-relative overflow-auto bg-white" >
            ...
            <div class="content-area">
                <div class="content-title-wrapper px-15 py-10">
                    <h2 class="content-title text-uppercase m-0">Quiz Result Details</h2>
                </div>
                                    ...
                                    <v-container class="p-0 position-relative overflow-hidden">
                                        ...
                                        <v-row>
                                            <v-col class="customer-data-table-wrapper">
                                                <v-data-table :headers="headers" :items="quizResultDetailsList" :search="search" :hide-default-footer=true  class="elevation-1" :items-per-page="20">
                                                    <template v-slot:item.username="{item}">
                                                        <span v-if="item.user">{{item.user.first_name}} {{item.user.last_name}}</span>
                                                        <span v-else>Unknown ({{item.user_id}})</span>
                                                    </template>

                                                    <template v-slot:item.status="{item}">
                                                        <span v-if="item.status==1" class=" text-green" >Passed</span>
    ...
    export default {
        ...
        methods: {
            ...
            getResultDetails(pageUrl)
            {
                ...
                pageUrl = pageUrl == undefined ? 'quiz-results-details' : pageUrl;
                ...
                axios.get(pageUrl+'?page='+this.pagination.current,
                    ...
                    .then(function (response) {
                        if(response.data.status_code === 200){
                            ...
                            _that.pagination.current     = response.data.quiz_result_details.current_page;
                            _that.pagination.total       = response.data.quiz_result_details.last_page;
                            _that.quizResultDetailsList  = response.data.quiz_result_details.data;
