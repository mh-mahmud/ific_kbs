<template>
    <div class="main-wrapper d-flex">
        ...
        <div class="main-content-wrapper w-100 position-relative overflow-auto bg-white" >
            ...
            <div class="content-area" >
                <div class="content-title-wrapper px-15 py-10">
                    <h2 class="content-title text-uppercase m-0">Interest Rate List</h2>
                </div>
                                    ...
                                    <v-container class="p-0 position-relative overflow-hidden">
                                        ...
                                        <v-row>
                                            <v-col class="customer-data-table-wrapper">
                                                <v-data-table :headers="headers" :items="interestRate" :search="search" :hide-default-footer=true  class="elevation-1" :items-per-page="20">
                                                    ...
                                                    <template v-slot:item.actions="{item}">
                                                        ...
                                                        <button class="btn btn-success ripple-btn right-side-common-form btn-xs mx-1"
                                                                @click="interest_rate_id=item.id, isEditCheck=true" v-if="checkPermission('category-edit')">
                                                            <i class="fas fa-pen"></i>
                                                        </button>
                                                        <button  class="btn btn-danger ripple-btn right-side-common-form btn-xs mx-1"
                                                                 @click="interest_rate_id=item.id, isDeleteCheck=true" v-if="checkPermission('category-delete')">
        ...
        <div class="right-sidebar-wrapper with-upper-shape fixed-top px-20 pb-30 pb-md-40 pt-70">
            ...
            <!--            add data-->
            <interestRateAdd v-if="isAddCheck" :isAddCheck="isAddCheck" @rate-slide-close="getAddDataFromChild"></interestRateAdd>
            <!--            edit data-->
            <interestRateEdit v-if="isEditCheck" :isEditCheck="isEditCheck" :interestRateId="interest_rate_id" @rate-slide-close="getEditDataFromChild"></interestRateEdit>
            <!--            delete data -->
                    ...
                    <div class="form-wrapper">
                        ...
                        <div class="row mt-50 mt-md-80">
                            <div class="col-md-12">
                                ...
                                <p class="text-center"> Confirmation for Deleting Menu</p>
                                <div class="form-group d-flex justify-content-center align-items-center">
                                    <button type="button" class="btn btn-danger text-white rounded-pill ripple-btn px-30 mx-2" @click="deleteLink()"><i class="fas fa-trash"></i> Confirm</button>
...
<script>
    ...
    import interestRateAdd from "@/components/interest-rate/interestRateAdd";
    import interestRateEdit from "@/components/interest-rate/interestRateEdit";
    ...
    export default {
        ...
        methods: {
            ...
            getInterestRate(pageUrl)
            {
                let _that = this;
                // let user_role;

                pageUrl = pageUrl == undefined ? 'interest-rate' : pageUrl;
                    ...
                    }).then(function (response)
                {
                    ...
                    {   
                        // console.log('Data:',response.data.link_list);
                        _that.pagination.current = response.data.interest_rate.current_page;
                        _that.pagination.total   = response.data.interest_rate.last_page;
                        _that.interestRate       = response.data.interest_rate.data;
            ...
            {
                let _that = this;
                axios.delete('interest-rate/delete',
                    {
                        data:
                            {
                                id  : this.interest_rate_id
                            },
                        headers: {
                            'Authorization': 'Bearer ' + localStorage.getItem('authToken')
                        },
                    ...
                    if (response.data.status_code == 200) {
                        ...
                        _that.getInterestRate();
                        _that.removingRightSideWrapper();
                        _that.error_message         = '';
                        _that.success_message       = "Interest Rate Deleted Successfully";
