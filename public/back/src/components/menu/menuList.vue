<template>
    <div class="main-wrapper d-flex">
        ...
        <div class="main-content-wrapper w-100 position-relative overflow-auto bg-white" >
            ...
            <div class="content-area" >
                <div class="content-title-wrapper px-15 py-10">
                    <h2 class="content-title text-uppercase m-0">Menu List</h2>
                </div>
                                            ...
                                            <v-col class="customer-data-table-wrapper">
                                                <v-data-table :headers="headers" :items="menuList" :search="search" :hide-default-footer=true  class="elevation-1" :items-per-page="20">
                                                    ...
                                                    <template v-slot:item.actions="{item}">
                                                        <!-- <button class="btn btn-primary ripple-btn right-side-common-form btn-xs mx-1" @click="isHistoryCheck=true, menu_info=item"
                                                                v-if="checkPermission('history-list')">
                                                            <i class="fas fa-book text-white"></i>
                                                        </button> -->
                                                        <button class="btn btn-success ripple-btn right-side-common-form btn-xs mx-1"
                                                                @click="menu_id=item.id, isEditCheck=true" v-if="checkPermission('category-edit')">
                                                            <i class="fas fa-pen"></i>
                                                        </button>
                                                        <button  class="btn btn-danger ripple-btn right-side-common-form btn-xs mx-1"
                                                                 @click="menu_id=item.id, isDeleteCheck=true" v-if="checkPermission('category-delete')">
        ...
        <div class="right-sidebar-wrapper with-upper-shape fixed-top px-20 pb-30 pb-md-40 pt-70">
            ...
            <!--            add data-->
            <MenuAdd v-if="isAddCheck" :isAddCheck="isAddCheck" @menu-slide-close="getAddDataFromChild"></MenuAdd>
            <!--            edit data-->
            <MenuEdit v-if="isEditCheck" :isEditCheck="isEditCheck" :menuId="menu_id" @menu-slide-close="getEditDataFromChild"></MenuEdit>
            <!--            delete data -->
                        ...
                        <div class="row mt-50 mt-md-80">
                            <div class="col-md-12">
                                ...
                                <div class="form-group d-flex justify-content-center align-items-center">
                                    <button type="button" class="btn btn-danger text-white rounded-pill ripple-btn px-30 mx-2" @click="deleteMenu()"><i class="fas fa-trash"></i> Confirm</button>
    ...
    export default {
        ...
        methods: {
            ...
            getMenuList(pageUrl)
            {
                ...
                pageUrl = pageUrl == undefined ? 'menus' : pageUrl;
                    ...
                    }).then(function (response)
                {
                    ...
                    {
                        _that.pagination.current = response.data.menu_list.current_page;
                        _that.pagination.total   = response.data.menu_list.last_page;
                        _that.menuList           = response.data.menu_list.data;
